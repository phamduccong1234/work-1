...
export class MaterialPage {
  ...
  constructor(page: Page) {
    ...
    this.xpathHeader = this.page.getByRole('heading', { name: 'Tài liệu học automation test', level: 1 });
  }
  ...
  async checkHeaderVisible(){
    await expect(this.xpathHeader).toBeVisible();
  }
...
export class PersonalNotesPage extends MaterialPage {
  ...
  async findNote(keyword: string) {
    ...
    for (let i = 0; i < count; i++) {
      await expect((await noteItem.nth(i).innerText()).toLowerCase()).toContain(
        keyword.toLowerCase(),
      );
    }
  }
